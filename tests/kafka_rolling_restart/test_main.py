...
@mock.patch(
    'kafka_utils.kafka_rolling_restart.main.check_preconditions',
    return_value={'host': False},
    autospec=True,
)
@mock.patch('kafka_utils.kafka_rolling_restart.main.execute', autospec=True)
def test_prechecks_failure(mock_execute, mock_precondition):
    with pytest.raises(main.PrecheckFailedException):
        main.ensure_preconditions_before_executing_restart(
            "kafka",
            "0.10.1.1",
            "hostname",
            "dummy_cmd",
