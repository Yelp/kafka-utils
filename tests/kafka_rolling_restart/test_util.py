...
from kafka_utils.kafka_rolling_restart import error
from kafka_utils.kafka_rolling_restart import util


@mock.patch.object(util.FuturesSession, 'get', autospec=True)
def test_read_cluster_value_partitions(mock_get):
    ...
    p, b = util.read_cluster_status(["host1", "host2", "host3"], 80, "jolokia")
...
def test_read_cluster_value_exit(mock_get):
    ...
    with pytest.raises(SystemExit):
        p, b = util.read_cluster_status(["host1"], 80, "jolokia")
...
def test_wait_for_stable_cluster_success(mock_sleep, mock_read):
    util.wait_for_stable_cluster([], 1, "", 5, 3, 100)
...
def test_wait_for_stable_cluster_timeout(mock_sleep, mock_read):
    with pytest.raises(error.WaitTimeoutException):
        util.wait_for_stable_cluster([], 1, "", 5, 3, 100)
